...
@st.cache_data
def carregar_dados():
    df = pd.read_excel('convenio_detalhado_linha.xlsx')
    df['Data'] = pd.to_datetime(df['Data'])
    df['Valor'] = [valor.replace(',', '.') for valor in df['Valor']]
    df['Valor'] = df['Valor'].astype('float')
    df['ano_mes'] = df['Data'].dt.to_period('M').astype('datetime64[ns]')
    return df
...
# Aplicações de filtros ao código:
with co4:
    # Inicializar session_state se não existir
    if 'chek_ontem' not in st.session_state:
        st.session_state.chek_ontem = False
    if 'chek_7_dias' not in st.session_state:
        st.session_state.chek_7_dias = False
    if 'chek_30_dias' not in st.session_state:
        st.session_state.chek_30_dias = False

    st.subheader('Avaliação dos exames e convênios:')

    chek_ontem = st.checkbox('Ontem',
                             key='chek_ontem',
                             disabled=st.session_state.chek_7_dias or st.session_state.chek_30_dias)

    chek_7_dias = st.checkbox('Últimos 7 dias',
                              key='chek_7_dias',
                              disabled=st.session_state.chek_ontem or st.session_state.chek_30_dias)

    chek_30_dias = st.checkbox('Últimos 30 dias',
                               key='chek_30_dias',
                               disabled=st.session_state.chek_ontem or st.session_state.chek_7_dias)
...
if chek_ontem:
    ...
    if convênio == 'Todos':
        ...
        with co1:
            ...
            st.dataframe(mais_vendidos_qtde.head(5), use_container_width=True)
        with co2:
            st.write('💰 Top 5 Exames Mais Lucrativos (Valor)')
            st.dataframe(mais_vendidos_valor.head(5), use_container_width=True)
        with co3:
        ...
        co1, co2, co3 = st.columns(3)
        with co1:
            st.write(f'🏆 Top 5 Exames Mais Vendidos - {convênio} (Quantidade)')
            st.dataframe(mais_vendidos_qtde.head(5), use_container_width=True)
        with co2:
            st.write(f'💰 Top 5 Exames Mais Lucrativos - {convênio} (Valor)')
            st.dataframe(mais_vendidos_valor.head(5), use_container_width=True)
        with co3:
...
if chek_7_dias:
    ...
    if convênio == 'Todos':
        ...
        with co1:
            st.write('🏆 Top 5 Exames Mais Vendidos (Quantidade)')
            st.dataframe(mais_vendidos_qtde_7.head(), use_container_width=True)
        with co2:
            st.write('💰 Top 5 Exames Mais Lucrativos (Valor)')
    ...
    mesclado_7 = pd.merge(qtde_df_7, valor_df_7, on='Descrição', how='outer')
...
if chek_30_dias:
    ...
    if convênio == 'Todos':
        ...
        # Top 5 exames mais vendidos por quantidade
        mais_vendidos_qtde_30 = filtro_30['Descrição'].value_counts().head(5)

        # Top 5 exames mais lucrativos por valor
        mais_vendidos_valor_30 = filtro_30.groupby(
            'Descrição')['Valor'].sum().sort_values(ascending=False).head(5)
            ...
            if mesclado_30['Valor_Total'].min() == mesclado_30['Valor_Total'].max():
                ...
                if convênio == 'LABORATORIO ASO':
                    ...
                    if 'filtro_30_qtde' in locals():
                        st.dataframe(mesclado_30[filtro_30_qtde & filtro1_30])
                    else:
                        st.dataframe(mesclado_30[filtro1_30])
    # Fim das faixas para 30 dias
...
st.divider()
# Mai frequentes e valiosos:
st.subheader('Top 10 exames mais frequentes e com maior valor em vendas')
...
chek_qtde = st.checkbox('filtrar por número de exames')
...
if not chek_qtde:
    ...
    if convênio == 'Todos':
        if lista_e == []:
            visualização = df.groupby('ano_mes')['Valor'].sum().reset_index()
            ...
            fig = px.bar(visualização, x='ano_mes',
                         y='Valor', text='Valor_Formatado', color='Descrição')
            fig.update_traces(
                textposition='outside',
                textfont=dict(size=12, color='black')
            )
        ...
        if lista_e == []:
            ...
            visualização = data.groupby(
                ['ano_mes', 'Descrição'])['Valor'].sum().reset_index()
...
st.subheader('Proporção do número de exames por tipo de convênio')
...
fig.update_traces(
    ...
    texttemplate='%{text:.1f}%',     # Formato: 1 casa decimal + %
